...
/////// Normals:
                float2 node_1219 = i.uv0;
                float4 node_1105 = tex2D(_NORMAL,TRANSFORM_TEX(node_1219.rg, _NORMAL));
                float2 node_1103 = (0.05*(node_1105.a - _HEIGHTOFFSET)*mul(tangentTransform, viewDirection).xy + node_1219.rg);
...
////// Specular:
                NdotL = max(0.0, NdotL);
                float4 node_515 = tex2D(_DETAIL,TRANSFORM_TEX(node_1219.rg, _DETAIL));
                float node_658 = ((pow((1.0 - tex2D(_DIFFUSE,TRANSFORM_TEX(node_1219.rg, _DIFFUSE)).a),4.0)+node_1105.a)*_DETAILBLEND);
                float node_527 = lerp(pow(tex2D(_DIFFUSE,TRANSFORM_TEX(node_1219.rg, _DIFFUSE)).a,1.5),pow(dot(node_515.rgb,float3(0.3,0.59,0.11)),2.0),node_658);
                ...
                finalColor += diffuseLight * lerp(tex2D(_DIFFUSE,TRANSFORM_TEX(node_1219.rg, _DIFFUSE)).rgb,node_515.rgb,node_658);
                finalColor += specular;
/// Final Color:
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);
/////// Normals:
                float2 node_1220 = i.uv0;
                float4 node_1105 = tex2D(_NORMAL,TRANSFORM_TEX(node_1220.rg, _NORMAL));
                float2 node_1103 = (0.05*(node_1105.a - _HEIGHTOFFSET)*mul(tangentTransform, viewDirection).xy + node_1220.rg);
                float3 normalLocal = (tex2D(_NORMAL,TRANSFORM_TEX(node_1103.rg, _NORMAL)).rgb*2.0+-1.0);
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
////// Specular:
                NdotL = max(0.0, NdotL);
                float4 node_515 = tex2D(_DETAIL,TRANSFORM_TEX(node_1220.rg, _DETAIL));
                float node_658 = ((pow((1.0 - tex2D(_DIFFUSE,TRANSFORM_TEX(node_1220.rg, _DIFFUSE)).a),4.0)+node_1105.a)*_DETAILBLEND);
                float node_527 = lerp(pow(tex2D(_DIFFUSE,TRANSFORM_TEX(node_1220.rg, _DIFFUSE)).a,1.5),pow(dot(node_515.rgb,float3(0.3,0.59,0.11)),2.0),node_658);
                float3 specularColor = float3(node_527,node_527,node_527);
                float3 specular = attenColor * pow(max(0,dot(halfDirection,normalDirection)),gloss) * specularColor;
                float3 finalColor = 0;
                float3 diffuseLight = diffuse;
                finalColor += diffuseLight * lerp(tex2D(_DIFFUSE,TRANSFORM_TEX(node_1220.rg, _DIFFUSE)).rgb,node_515.rgb,node_658);
                finalColor += specular;
/// Final Color:
